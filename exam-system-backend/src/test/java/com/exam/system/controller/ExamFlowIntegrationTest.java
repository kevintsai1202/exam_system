...
class ExamFlowIntegrationTest {
    ...
    /** 目前登入使用者的 JWT Token，由 setUpAuth() 在每個測試前設定 */
    private String instructorToken;

    /** startExam 回傳的講師 Session ID，用於後續 startQuestion / endExam 呼叫 */
    private String instructorSessionId;

    /**
     * 每個測試前：透過 MockMvc 註冊帳號並取得 JWT Token
     * 註：@Transactional 測試中 @BeforeEach 與測試方法共享同一事務，資料可見且測試結束後一起 rollback
     */
    @BeforeEach
    void setUpAuth() throws Exception {
        EmailRegisterRequestDTO req = new EmailRegisterRequestDTO();
        req.setName("Integration Test User");
        req.setEmail("it-user@test.local");
        ...
        AuthTokenResponseDTO tokenResponse = objectMapper.readValue(
                registerResult.getResponse().getContentAsString(), AuthTokenResponseDTO.class);
        instructorToken = tokenResponse.getToken();
    }

    /**
     * 為需要認證的 MockMvc 請求加上 Bearer JWT 標頭
     */
    private MockHttpServletRequestBuilder withAuth(MockHttpServletRequestBuilder req) {
        return req.header("Authorization", "Bearer " + instructorToken);
    }

    /**
     * 為 startQuestion / endExam 等需要 Instructor-Session-Id 的請求加上 header
     */
    private MockHttpServletRequestBuilder withSession(MockHttpServletRequestBuilder req) {
        return req.header("Instructor-Session-Id", instructorSessionId);
    }
    ...
    void testScenario1_InstructorCreatesExam() throws Exception {
        ...
        MvcResult createResult = mockMvc.perform(withAuth(post("/api/exams"))
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        // === 步驟 2：查詢測驗詳情 ===
        mockMvc.perform(withAuth(get("/api/exams/" + createdExam.getId())))
                .andDo(print())
                .andExpect(status().isOk())
        ...
        // === 步驟 3：查詢測驗的題目列表（回傳 {examId, totalQuestions, questions:[]}）===
        mockMvc.perform(get("/api/exams/" + createdExam.getId() + "/questions"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.questions").isArray())
                .andExpect(jsonPath("$.questions.length()").value(3))
                .andExpect(jsonPath("$.questions[0].questionText").value("什麼是 Spring Boot？"))
                .andExpect(jsonPath("$.questions[0].options.length()").value(4))
                .andExpect(jsonPath("$.questions[1].questionText").value("Spring Boot 的主要優勢是？"))
                .andExpect(jsonPath("$.questions[2].questionText").value("如何啟動 Spring Boot 應用？"));
    }
    ...
    void testScenario2_InstructorStartsExam() throws Exception {
        ...
        // === 步驟 1：啟動測驗（注意：startExam 是 PUT）===
        mockMvc.perform(withAuth(put("/api/exams/" + createdExam.getId() + "/start"))
    ...
    void testScenario3_StudentsJoinExam() throws Exception {
        ...
        invalidStudent.setLocation("TPE");
    ...
    void testScenario4_CompleteAnswerFlow() throws Exception {
        ...
        // === 步驟 2：講師開始第一題（注意：startQuestion 是 PUT + Instructor-Session-Id header）===
        mockMvc.perform(withSession(put("/api/exams/" + examId + "/questions/0/start")))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("題目已開始"))
                .andExpect(jsonPath("$.questionIndex").value(0));

        // 取得第一題的題目資訊（回傳格式：{examId, totalQuestions, questions:[]}）
        MvcResult questionsResult = mockMvc.perform(get("/api/exams/" + examId + "/questions"))
                .andExpect(status().isOk())
                .andReturn();

        List<QuestionDTO> questions = parseQuestionsFromResponse(questionsResult);
        ...
        // === 步驟 5：講師開始第二題 ===
        mockMvc.perform(withSession(put("/api/exams/" + examId + "/questions/1/start")))
                .andExpect(status().isOk())
    ...
    void testScenario5_EndExamAndLeaderboard() throws Exception {
        ...
        // 第三題：只有學員1答對
        mockMvc.perform(withSession(put("/api/exams/" + examId + "/questions/2/start")))
                .andExpect(status().isOk());
        ...
        // === 步驟 5：講師結束測驗（PUT + Instructor-Session-Id）===
        mockMvc.perform(withSession(withAuth(put("/api/exams/" + examId + "/end"))))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("測驗已結束"));

        // === 步驟 6：驗證測驗狀態已更新為 ENDED ===
        mockMvc.perform(withAuth(get("/api/exams/" + examId)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("ENDED"))
                .andExpect(jsonPath("$.endedAt").exists());

        // === 步驟 7：查詢排行榜 ===
        mockMvc.perform(withAuth(get("/api/statistics/exams/" + examId + "/leaderboard")))
                .andDo(print())
                .andExpect(status().isOk())
    ...
    /**
     * 解析 GET /api/exams/{id}/questions 回傳的包裝物件中的題目清單
     * 回傳格式：{examId, totalQuestions, questions: [...]}
     */
    private List<QuestionDTO> parseQuestionsFromResponse(MvcResult result) throws Exception {
    ...
    /**
     * 建立並啟動測驗（輔助方法）
     * 同時將 instructorSessionId 存入欄位，供後續 startQuestion / endExam 使用
     */
    private ExamDTO createAndStartExam() throws Exception {
        ...
        // startExam 是 PUT
        MvcResult startResult = mockMvc.perform(withAuth(put("/api/exams/" + createdExam.getId() + "/start"))
                        .param("baseUrl", "http://localhost:5173"))
                .andExpect(status().isOk())
                .andReturn();

        ExamDTO startedExam = objectMapper.readValue(
                startResult.getResponse().getContentAsString(), ExamDTO.class);
        // 儲存 instructorSessionId 供後續 startQuestion / endExam 使用
        instructorSessionId = startedExam.getInstructorSessionId();
        return startedExam;
    }
}
