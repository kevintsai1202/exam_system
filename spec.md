...
- 視覺可用性補充：
  - 黑夜模式下，離島（澎湖、金門、連江）配色需維持高於背景的明度與描邊對比。
  - 不可讓離島在深色背景中接近背景色而難以辨識。
...
### 34.1 管理員刪除使用者（2026-03-09）
- 管理員頁 `/admin/users` 需提供刪除使用者操作。
- 最小改動策略：
  - 沿用既有 `User` 與 admin 使用者清單 API。
  - 新增 admin 專用刪除 API，刪除成功後前端重新載入使用者列表。
- 行為規則：
  - 只有 `ADMIN` 可刪除使用者。
  - 目前登入中的管理員不得刪除自己的帳號，避免當前 session 立即失效並造成管理入口不可用。
  - 前端在送出刪除前需顯示明確確認訊息，避免誤刪。
- 視覺與互動：
  - 刪除按鈕需與升級角色操作區分，使用高風險風格。
  - 刪除成功後，該筆使用者需自列表移除或重新載入後不再顯示。

```mermaid
flowchart TD
    A[ADMIN 進入 /admin/users] --> B[讀取使用者列表]
    B --> C[點擊刪除使用者]
    C --> D{是否為目前登入管理員自己}
    D -->|是| E[拒絕刪除並顯示錯誤]
    D -->|否| F[顯示確認對話框]
